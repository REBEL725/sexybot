...
async def gban(userbot):
    ...
    if not sender.id == hum.id:
        ...
        REBELBOT = await edit_or_reply(
            ids, "`JANAB ABB APP KII KALLI GAND FAT JAYAGE🖕🖕🤣`"
        )
    hum = await userbot.client.get_me()
    await REBELBOT.edit(
        f"`🔥BETA BAAP SAAA PANAGA NAHI ABB DEKHO KYA HOO GAAA TERA. ABBB TERI KALLI GAND KO FAR DUU GA`"
    )
...
async def gunban(userbot):
    ...
    if user:
        if user.id == 1418571871:
            return await REBELBOT.edit(
                "**SUNO TERI KALLI GAND FAR DUU TU  APNA BAAP KOO GMUTE KARA GAA RANDI SALA OKAD BANA PHALA**"
            )
        try:
